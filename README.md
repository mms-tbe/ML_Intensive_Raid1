...
Модель машинного обучения, которая помогает маркетинговой команде заранее находить
клиентов с высоким риском оттока (`Churn`), чтобы обзванивать их **до** ухода.

## Бизнес-контекст и почему это влияет на метрику

Стоимость **пропущенного оттока** (false negative — не позвонили тому, кто ушёл)
примерно в **5 раз выше** стоимости лишнего звонка довольному клиенту (false positive).
Эта асимметрия определяет весь дизайн: мы оптимизируем **recall** по классу `churn`
...
**ни одного** churn'ера. Поэтому accuracy здесь бессмысленна.

Это иллюстрирует канонический пример из Chip Huyen, *Designing ML Systems* (гл. 4):
две модели для задачи CANCER/NORMAL имеют одинаковую accuracy 0.9, но одна ловит
10/100 раковых случаев, а другая — 90/100. Одинаковая accuracy, принципиально разная
...
- **Основная метрика отбора и тюнинга — PR-AUC (`average_precision`).** При сильном
  дисбалансе precision-recall-кривая информативнее ROC (Davis & Goadrich; Huyen гл. 4),
  т.к. ROC «льстит» за счёт большого числа true negatives.
...
1. **`TotalCharges` грузится как `object`** — в 11 строках стоит пустая строка `" "`
   (у клиентов с `tenure=0`). Лечится `pd.to_numeric(errors='coerce')` → `NaN`
...
2. **Подсчёт активных сервисов (`n_services`)** — колонки сервисов содержат не только
   ...
   Наивный счёт `== "Yes"` молча обнулил бы `InternetService` — ловушка обойдена.
...
## Архитектура и защита от утечки
...
notebook/    EDA.ipynb (не оценивается)
